...
namespace App\Livewire\Pages\ArchiveOuts;
...
#[Title('Archive Out')]
class ArchiveOutsIndex extends Component
{
    ...
    public function changeModalMode($mode = "", $id = 0)
    {
        ...
        if ($id != 0 && ($mode == "update" || $mode == "show")) {
            $user_project = ArchiveOut::find($id);
            ...
            $this->form->addressed_to = $user_project->source;
    ...
    public function store()
    {
        ...
        $this->file->storeAs('archive-out', $name);
        $this->form->archive_file = "archive-out/" . $name;
        ArchiveOut::create((array)$this->form);
    ...
    public function update()
    {
        ...
        $this->file->storeAs('archive-out', $name);
        $this->form->archive_file = "archive-out/" . $name;
        $user_project = ArchiveOut::find($this->form->id);
    ...
    {
        $user_project = ArchiveOut::find($this->user_project->id);
        $user_project->update([
    ...
    {
        $this->user_project = ArchiveOut::find($id);
    ...
    {
        $datas = DB::table("archive_outs")->selectRaw("archive_outs.*,
        archive_categories.name as archive_categories_name, archive_out_statuses.name as status_name")
            ->join("archive_categories", "archive_outs.archive_category_id", "archive_categories.id")
            ->join("archive_out_statuses", "archive_outs.status_id", "archive_out_statuses.id");
        ...
        return view('livewire.pages.archive-outs.archive-outs-index', [
            'datas' => $datas,
            'statuses' => $statuses,
